    ...
    pub fn get_variables(&self) -> Vec<Variable> {
        ...
        match self {
            ...
            Instruction::Add(iadd) => {
                variables.push(iadd.des.clone());
                if let Operand::Variable(v) = &iadd.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &iadd.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Assign(iassign) => {
                variables.push(iassign.des.clone());
                if let Operand::Variable(v) = &iassign.src {
                    variables.push(v.clone());
                }
            }
            Instruction::Alloc(ialloc) => {
                variables.push(ialloc.des.clone());
                if let Operand::Variable(v) = &ialloc.size {
                    variables.push(v.clone());
                }
            }
            Instruction::Call(icall) => {
                if let Some(des) = icall.des.as_ref() {
                    variables.push(des.clone());
                }
                for arg in &icall.args {
                    if let Operand::Variable(v) = arg {
                        variables.push(v.clone());
                    }
                }
            }
            Instruction::Cmp(icmp) => {
                variables.push(icmp.des.clone());
                if let Operand::Variable(v) = &icmp.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &icmp.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Copy(icopy) => {
                variables.push(icopy.des.clone());
                if let Operand::Variable(v) = &icopy.src {
                    variables.push(v.clone());
                }
            }
            Instruction::ElemGet(ielem_get) => {
                variables.push(ielem_get.des.clone());
                if let Operand::Variable(v) = &ielem_get.ptr {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &ielem_get.index {
                    variables.push(v.clone());
                }
            }
            Instruction::ElemSet(ielem_set) => {
                variables.push(ielem_set.addr.clone());
                if let Operand::Variable(v) = &ielem_set.index {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &ielem_set.value {
                    variables.push(v.clone());
                }
            }
            Instruction::And(iand) => {
                variables.push(iand.des.clone());
                if let Operand::Variable(v) = &iand.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &iand.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Or(ior) => {
                variables.push(ior.des.clone());
                if let Operand::Variable(v) = &ior.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &ior.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::XOr(ixor) => {
                variables.push(ixor.des.clone());
                if let Operand::Variable(v) = &ixor.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &ixor.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Gt(igt) => {
                variables.push(igt.des.clone());
                if let Operand::Variable(v) = &igt.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &igt.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Gte(igte) => {
                variables.push(igte.des.clone());
                if let Operand::Variable(v) = &igte.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &igte.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Lt(ilt) => {
                variables.push(ilt.des.clone());
                if let Operand::Variable(v) = &ilt.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &ilt.rhs {
                    variables.push(v.clone());
                }
            }
            ...
            Instruction::Load(iload) => {
                variables.push(iload.des.clone());
                if let Operand::Variable(v) = &iload.src {
                    variables.push(v.clone());
                }
            }
            Instruction::Mul(imul) => {
                variables.push(imul.des.clone());
                if let Operand::Variable(v) = &imul.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &imul.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Phi(iphi) => {
                variables.push(iphi.des.clone());
                for (_, v) in &iphi.branches {
                    variables.push(v.clone());
                }
            }
            Instruction::Return(ireturn) => {
                if let Operand::Variable(v) = &ireturn.src {
                    variables.push(v.clone());
                }
            }
            Instruction::Sub(isub) => {
                variables.push(isub.des.clone());
                if let Operand::Variable(v) = &isub.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &isub.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::Div(idiv) => {
                variables.push(idiv.des.clone());
                if let Operand::Variable(v) = &idiv.lhs {
                    variables.push(v.clone());
                }
                if let Operand::Variable(v) = &idiv.rhs {
                    variables.push(v.clone());
                }
            }
            Instruction::IfElse(..) => unreachable!("Should lower before calling this method"),
            Instruction::Loop(..) => unreachable!("Should lower before calling this method"),
        }
        variables
    }
}
